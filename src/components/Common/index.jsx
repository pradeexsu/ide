export const Button = ({ iconUrl, text, isPrimary }) =>
  isPrimary ? (
    <button className="my-auto mr-2 flex h-8 select-none gap-1 rounded-full border-none bg-pink-500/60 py-1 px-4 text-white duration-200 hover:bg-purple-400/50 active:bg-red-600/75">
      <img src={iconUrl} className="mr-1 h-5" />
      {text}
    </button>
  ) : (
    <button className="my-auto mr-2 flex h-8 select-none rounded-full border-none bg-white/5 py-[6px] px-[14px]   text-white hover:bg-white/10">
...
export const ButtonIcon = ({ iconUrl }) => (
